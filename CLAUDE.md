...
사용자가 승인하면 create-presentation 스킬을 사용하여 프레젠테이션을 생성한다. 대규모 프레젠테이션(16장+)은 `.claude/agents/presentation-builder.md` 에이전트 파이프라인을 자동 사용한다. 사용자가 거부하거나 무시하면 리서치로 마무리한다.

### 프레젠테이션 생성 규칙

리서치 기반 프레젠테이션은 아래 규칙을 따른다:

- **슬라이드 수**: Researcher 2명 이하 → 30장 이상, 3명 이상 → 50장 이상
- **콘텐츠 소스**: 리서치 폴더의 보고서만 사용. 임의 내용 추가 금지
- **용어 정책**: 전문 약어는 슬라이드 제목에 쓰지 않는다. 본문에서 처음 등장할 때 풀어서 설명한다
- **저장 위치**: `presentation-build/{date}-{topic}/`
  - 예: `presentation-build/2026-03-20-mason-ontology-whitepaper/`
  - 생성 코드(JS 파일)와 결과물(PPTX)을 같은 폴더에 저장
