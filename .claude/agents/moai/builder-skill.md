...
- Validate YAML frontmatter structure (see Frontmatter Format Rules below)
...
### Frontmatter Format Rules [HARD]

[HARD] YAML frontmatter MUST follow this exact structure:
...
Rules:
- [HARD] Line 1 MUST be exactly `---` (opening delimiter)
- [HARD] Line 2 MUST be a YAML field (e.g., `name:`). NO blank line after opening `---`
- [HARD] Closing `---` MUST exist on its own line after all YAML fields
- [HARD] NO blank lines inside the frontmatter block (between opening and closing `---`)
- [HARD] After writing SKILL.md, Read it back and verify lines 1-2 are `---` then `name:`
- [HARD] Use Write tool (not Edit) when creating SKILL.md to prevent linter interference
- [HARD] Never use `##` or any markdown syntax inside frontmatter. YAML only between `---` delimiters
